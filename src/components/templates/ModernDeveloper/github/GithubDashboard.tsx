...
const DEFAULT_LAYOUT = ['summary', 'coach', 'insights', 'graph', 'repos', 'languages', 'timeline'];

const WIDGET_NAMES: Record<string, string> = {
  summary: 'Developer Profile',
  coach: 'Portfolio Coach',
...
export default function GithubDashboard({ username }: Props) {
  ...
  const renderWidget = (id: string) => {
    ...
    switch (id) {
      ...
      case 'coach':
        return <PortfolioCoach stats={stats} profile={profile} repos={repos} />;
      case 'insights':
        return <DeveloperInsights stats={stats} repos={repos} />;
      case 'graph':
        return <ContributionGraph username={username!} stats={stats} />;
      case 'repos':
        return <FeaturedRepositories repos={repos} />;
      case 'languages':
  ...
  const getWidgetClass = (id: string) => {
    switch (id) {
      ...
      case 'coach': return 'md:col-span-12 xl:col-span-6';
      case 'insights': return 'md:col-span-12 xl:col-span-6';
      ...
      case 'timeline': return 'md:col-span-12 xl:col-span-12';
  ...
  return (
    <section id="github" className="py-24 relative bg-black selection:bg-primary/30 min-h-screen">
      ...
      <div className="absolute inset-0 bg-black/40 backdrop-blur-xl border-y border-white/5 pointer-events-none" />
      
      {/* Sidebar for Edit Mode */}
      <div className={`fixed inset-y-0 right-0 w-80 bg-[#0a0a0a] border-l border-white/10 z-[100] transform transition-transform duration-500 ease-in-out ${isEditMode ? 'translate-x-0 shadow-2xl' : 'translate-x-full'}`}>
