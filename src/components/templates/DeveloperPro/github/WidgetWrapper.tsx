...
export function WidgetWrapper({ id, children, isEditMode, onHide, className = '' }: Props) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`relative h-full transition-all duration-300 ${isEditMode ? 'ring-1 ring-[rgba(255,255,255,0.06)] ring-offset-4 ring-offset-[#09090B] rounded-sm opacity-90 hover:opacity-100' : ''} ${isDragging ? 'scale-[1.02] shadow-2xl shadow-black ring-[#2DD4BF]' : ''} ${className}`}
    >
      {isEditMode && (
        <>
          {/* Drag Handle */}
