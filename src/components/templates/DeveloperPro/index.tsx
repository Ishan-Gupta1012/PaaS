...
function EchoHeading({ text }: { text: string }) {
  return (
    <div style={{ position: "relative", marginBottom: "48px", paddingTop: "16px", userSelect: "none" }}>
      <span aria-hidden="true" style={{ position: "absolute", top: "-14px", left: "-2px", fontSize: "96px", fontWeight: 900, lineHeight: 1, letterSpacing: "-0.04em", color: "transparent", WebkitTextStroke: "1px rgba(45,212,191,0.07)", pointerEvents: "none", whiteSpace: "nowrap" }}>{text}</span>
      <h2 style={{ position: "relative", fontSize: "48px", fontWeight: 700, lineHeight: 1.1, letterSpacing: "-0.03em", color: "#2DD4BF", margin: 0 }}>{text}</h2>
    </div>
  );
}
...
export default function DeveloperProTemplate({ data }: Props) {
      ...
      try {
        await fetch('/api/contact', {
          ...
          body: JSON.stringify({
            ...
            formName: 'Developer Pro Template Contact Form',
          })
        });
      } catch (err) {
        console.error(err);
  ...
  const NAV = [
    ...
    ...(data.hero.githubUsername && data.hero.showGithub !== false ? [{ id: "github", label: "Github" }] : []),
  ...
  return (
    <div style={{ backgroundColor: C.bg, color: C.text, fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif", minHeight: "100vh", WebkitFontSmoothing: "antialiased" }}>
      <style>{`
        ...
        @keyframes se-ring-a { from{transform:rotateX(65deg) rotateY(15deg) rotateZ(0deg)} to{transform:rotateX(65deg) rotateY(15deg) rotateZ(360deg)} }
        @keyframes se-ring-b { from{transform:rotateX(65deg) rotateY(15deg) rotateZ(0deg)} to{transform:rotateX(65deg) rotateY(15deg) rotateZ(-360deg)} }
        .r1{animation:se-ring-a 6s linear infinite}
        .r2{animation:se-ring-b 10s linear infinite}
        .r3{animation:se-ring-a 14s linear infinite}
        ...
        .se-input{width:100%;padding:14px 18px;border-radius:12px;font-size:16px;font-family:inherit;background:#0D0D10;border:1px solid rgba(255,255,255,.08);color:#fff;outline:none;transition:border-color .2s,box-shadow .2s}
      ...
      <section id="home" style={{ minHeight: "100vh", display: "flex", alignItems: "center", position: "relative", paddingTop: "72px", overflow: "hidden", scrollMarginTop: "100px" }}>
        <div aria-hidden="true" style={{ position: "absolute", inset: 0, backgroundImage: "url('https://i.postimg.cc/cLBz4Hd5/Screenshot-2026-07-26-at-5-35-09-PM.png')", backgroundSize: "cover", backgroundPosition: "center", filter: "blur(120px)", opacity: 0.25, pointerEvents: "none" }} />
        ...
        <div style={{ maxWidth: "1140px", width: "100%", margin: "0 auto", padding: "80px 32px", position: "relative" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "64px", alignItems: "center" }} className="se-g2">
            <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
              <div className="se-f1">
                <span style={{ fontSize: "13px", fontWeight: 500, letterSpacing: ".18em", textTransform: "uppercase", color: C.secondary }}>
                  I&apos;m {name}
                </span>
              </div>
              <div className="se-f2">
                <h1 style={{ fontSize: "clamp(48px,7vw,84px)", fontWeight: 900, lineHeight: 1.04, letterSpacing: "-0.04em", margin: 0 }}>
              ...
              <p className="se-f3" style={{ fontSize: "18px", lineHeight: 1.75, color: C.secondary, maxWidth: "500px", margin: 0 }}>
                {bio}
              </p>
              ...
              <div className="se-f5" style={{ display: "flex", gap: "40px", paddingTop: "20px", borderTop: "1px solid rgba(255,255,255,.06)", marginTop: "4px" }}>
                {[
                  ...
                  <div key={s.l}>
                    <div style={{ fontSize: "28px", fontWeight: 800, color: C.text, letterSpacing: "-0.03em" }}>{s.v}</div>
                    <div style={{ fontSize: "14px", color: C.muted, marginTop: "2px", fontWeight: 500 }}>{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
            <div className="se-r" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div className="se-fl" style={{ position: "relative", width: "360px", height: "360px" }}>
                {[0, 1, 2].map(i => (
                  <div key={i} className={`r${i + 1}`} style={{ position: "absolute", inset: `${-(i * 80) - 40}px`, borderRadius: "50%", border: `1.5px solid rgba(45,212,191,${0.18 - i * 0.05})`, transformStyle: 'preserve-3d' }} />
                ))}
                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {data.hero?.avatarUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={data.hero.avatarUrl} alt={data.hero.logoText || "Profile"} style={{ width: "240px", height: "240px", borderRadius: "50px", border: "3px solid rgba(45,212,191,.25)", objectFit: "cover", zIndex: 10, boxShadow: "0 0 40px rgba(45,212,191,0.15)" }} />
                  ) : (
                    <div style={{ width: "240px", height: "240px", borderRadius: "50px", backgroundColor: "rgba(45,212,191,.08)", border: "3px solid rgba(45,212,191,.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "64px", fontWeight: 900, color: "#2DD4BF", letterSpacing: "-0.03em", zIndex: 10, boxShadow: "0 0 40px rgba(45,212,191,0.15)" }}>
                      {initials}
                    </div>
                  )}
                </div>
                {heroSkills.map((l, i) => {
                  ...
                  const r = 320, rad = (a * Math.PI) / 180, x = Math.cos(rad) * r, y = Math.sin(rad) * r;
                  return <div key={l} style={{ position: "absolute", left: `calc(50% + ${x}px)`, top: `calc(50% + ${y}px)`, transform: "translate(-50%,-50%)", padding: "8px 18px", borderRadius: "10px", fontSize: "16px", fontWeight: 700, backgroundColor: "rgba(45,212,191,.08)", color: "#2DD4BF", border: "1px solid rgba(45,212,191,.18)", whiteSpace: "nowrap", zIndex: 20 }}>{l}</div>;
                })}
              </div>
            </div>
          </div>
      ...
      {/* PROJECTS */}
      {data.projects && data.projects.length > 0 && (
        <section id="projects" style={{ padding: "80px 0", borderBottom: "1px solid rgba(255,255,255,.04)", scrollMarginTop: "100px" }}>
          <div style={{ maxWidth: "1140px", margin: "0 auto", padding: "0 32px" }}>
            ...
            {data.projects.map((p, idx) => (
              <div key={p.id} className="se-pr" onMouseEnter={() => setHoveredProject(p.id)} onMouseLeave={() => setHoveredProject(null)}>
                <div style={{ display: "grid", gridTemplateColumns: "44px 1fr 44px", gap: "32px", alignItems: "start", position: "relative" }}>
                  <span style={{ fontSize: "44px", fontWeight: 900, lineHeight: 1, letterSpacing: "-0.04em", color: hoveredProject === p.id ? "#2DD4BF" : "rgba(255,255,255,.07)", transition: "color .3s", paddingTop: "4px" }}>
                    {String(idx + 1).padStart(2, '0')}
                  </span>
                  <div>
                    <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "10px", marginBottom: "8px" }}>
                      <h3 style={{ fontSize: "24px", fontWeight: 700, color: C.text, letterSpacing: "-0.02em", margin: 0 }}>{p.name}</h3>
                      {p.techStack.map(t => <span key={t} style={{ padding: "4px 10px", borderRadius: "5px", fontSize: "12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.07)", color: C.muted }}>{t}</span>)}
                    </div>
                    <p style={{ fontSize: "14px", color: C.muted, marginBottom: "12px", fontWeight: 500 }}>Project</p>
                    <p style={{ fontSize: "16px", lineHeight: 1.7, color: C.secondary, marginBottom: "16px" }}>{p.description}</p>
                    {p.highlights && p.highlights.length > 0 && (
                      <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "12px" }}>
                        {p.highlights.map((b, i) => (
                          <li key={i} style={{ display: "flex", alignItems: "flex-start", gap: "12px", fontSize: "16px", lineHeight: 1.7, color: C.secondary }}>
                            <span style={{ marginTop: "10px", width: "5px", height: "5px", borderRadius: "50%", backgroundColor: "#2DD4BF", flexShrink: 0, opacity: .7 }} />
                            {b}
                          </li>
                        ))}
                      </ul>
      ...
      {(experienceList.length > 0 || achievementsList.length > 0) && (
        <section id="experience" style={{ padding: "80px 0", borderBottom: "1px solid rgba(255,255,255,.04)", scrollMarginTop: "100px" }}>
          <div style={{ maxWidth: "1140px", margin: "0 auto", padding: "0 32px" }}>
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    {experienceList.map((exp, i) => (
                      <div key={exp.id} style={{ paddingLeft: "32px", paddingBottom: i < experienceList.length - 1 ? "40px" : 0, position: "relative" }}>
                        <div style={{ position: "absolute", left: "-5px", top: "8px", width: "11px", height: "11px", borderRadius: "50%", backgroundColor: C.bg, border: "2px solid #2DD4BF" }} />
                        <span style={{ display: "block", fontSize: "13px", fontWeight: 600, color: C.muted, marginBottom: "6px", letterSpacing: ".06em" }}>{exp.startDate} - {exp.endDate}</span>
                        <h3 style={{ fontSize: "20px", fontWeight: 700, color: C.text, marginBottom: "6px", letterSpacing: "-0.01em" }}>{exp.title}</h3>
                        <p style={{ fontSize: "16px", fontWeight: 600, color: "#2DD4BF", marginBottom: "4px" }}>{exp.organization}</p>
                        <p style={{ fontSize: "14px", color: C.muted }}>{location}</p>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                    {achievementsList.map((a, idx) => (
                      <div key={a.id} className="se-ach">
                        <span style={{ fontSize: "36px", fontWeight: 900, color: "rgba(45,212,191,.18)", lineHeight: 1, letterSpacing: "-0.04em", flexShrink: 0, paddingTop: "2px" }}>{String(idx + 1).padStart(2, '0')}</span>
                        <div>
                          <p style={{ fontSize: "13px", fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: C.text, marginBottom: "6px" }}>{a.title}</p>
                          <p style={{ fontSize: "14px", color: C.muted, marginBottom: "4px" }}>{a.organization}</p>
                          <p style={{ fontSize: "13px", fontWeight: 600, color: "#2DD4BF" }}>{a.startDate}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {/* GITHUB ACTIVITY */}
      {data.hero.githubUsername && data.hero.showGithub !== false && (
        <GithubDashboard username={data.hero.githubUsername} />
      )}

      {/* CONTACT */}
      <section id="contact" style={{ padding: "80px 0", scrollMarginTop: "100px" }}>
        <div style={{ maxWidth: "1140px", margin: "0 auto", padding: "0 32px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "80px", alignItems: "start" }} className="se-g2">
            <div>
              <EchoHeading text="Contact" />
              <p style={{ fontSize: "17px", lineHeight: 1.8, color: C.secondary, maxWidth: "420px", marginBottom: "36px" }}>
              ...
              {data.hero.socials && data.hero.socials.length > 0 && (
                <div style={{ display: "flex", gap: "16px", marginBottom: "48px" }}>
                  {data.hero.socials.map(s => (
                    <a key={s.platform} href={s.url} target="_blank" rel="noopener noreferrer" aria-label={s.platform} style={{ width: "48px", height: "48px", borderRadius: "14px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", fontWeight: 800, color: "#2DD4BF", backgroundColor: "rgba(45,212,191,.08)", border: "1px solid rgba(45,212,191,.2)", textDecoration: "none", transition: "transform .2s,box-shadow .2s" }}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                {[{ k: "Email", v: email }, { k: "Location", v: location }, { k: "Status", v: "Open to opportunities" }].map(item => (
                  <div key={item.k} style={{ display: "flex", alignItems: "center", gap: "20px" }}>
                    <span style={{ fontSize: "13px", fontWeight: 600, color: C.muted, textTransform: "uppercase", letterSpacing: ".1em", width: "84px", flexShrink: 0 }}>{item.k}</span>
                    <span style={{ fontSize: "15px", color: C.secondary }}>{item.v}</span>
                  </div>
                ))}
              </div>
            </div>
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
              {[
                ...
                <div key={field.key}>
                  <label style={{ display: "block", fontSize: "13px", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".12em", color: C.muted, marginBottom: "10px" }}>{field.label}</label>
              ...
              <div>
                <label style={{ display: "block", fontSize: "13px", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".12em", color: C.muted, marginBottom: "10px" }}>Message</label>
